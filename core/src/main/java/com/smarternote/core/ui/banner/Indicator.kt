...
class IndicatorView @JvmOverloads constructor(
  ...
  override fun getParams(): LayoutParams {
    if (params == null) {
      params = LayoutParams(
  ...
  private fun drawCircleRect(canvas: Canvas, midY: Float) {
    ...
    var distance = indicatorSpacing + Math.max(this.ratioRadius, ratioRadius) * 2
    val leftX: Float
    val rightX: Float
  ...
  private fun indicatorStartX(index: Int): Float {
    ...
    /*
         ...
         */return centerX + if (indicatorStyle == IndicatorStyle.INDICATOR_DASH) 0 else (ratioIndicatorRadius - ratioRadius) / 2
  }

  private val ratioRadius: Float
    private get() = indicatorRadius * indicatorRatio
  private val ratioSelectedRadius: Float
    private get() = indicatorSelectedRadius * indicatorSelectedRatio
  ...
  fun setParams(params: LayoutParams?): IndicatorView {
    this.params = params
    return this
  }
}